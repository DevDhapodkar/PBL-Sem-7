...
**Traditional methods (baselines):** the same detectors and the same Stage-3
cross-modal validation, changing only the alignment — (a) **no alignment at all**
(overlay the two geocoded point sets as-is, `run_no_alignment`), the conventional
approach our idea replaces, and (b) a single **ICP** registration (no outlier
model). Holding Stage 3 fixed isolates the contribution of the RANSAC+CPD
point-set alignment. The head-to-head is in §8.2 and the
[live dashboard](docs/dashboard.html).
...
Metrics table + precision–recall curves for all methods, and average
precision. The proposed method (RANSAC+CPD) dominates the high-precision region.

### 7.1 The "with vs without alignment" comparison dashboard

A second, **self-contained** UI ([`docs/dashboard.html`](docs/dashboard.html),
generated by [`docs/build_dashboard.py`](docs/build_dashboard.py)) is dedicated to
the headline question of this project: *does the point-set alignment actually help?*
It is a single static HTML page (no server) that reads the pipeline's own result
files, so its numbers can never drift from the code. It renders in light and dark:

![Comparison dashboard — header & KPIs](docs/img/ui_hero.png)

**How it works** — the pipeline as chips, with the one step the traditional method
skips highlighted:

![Comparison dashboard — pipeline](docs/img/ui_pipeline.png)

**The comparison** — the four-method table plus F1 / recall bars, on identical
scenes with the fusion held fixed:

![Comparison dashboard — comparison table & bars](docs/img/ui_compare.png)

**Robustness** — precision as modality-specific clutter rises:

![Comparison dashboard — robustness](docs/img/ui_robust.png)

**Live Nagpur data** — the real Sentinel-1 × Sentinel-2 overlay, a per-lake table,
and a match-tolerance sweep showing the alignment keeps true corroborations that a
tight tolerance would otherwise drop:

![Comparison dashboard — live Nagpur data](docs/img/ui_real.png)

> Regenerate any time with `python docs/build_dashboard.py` after `run_demo.py`.

---

## 8. Results

### 8.0 How the comparison was set up (methodology)

The whole point of the study is to measure **what the point-set alignment adds**,
so the comparison is designed to change *only* the alignment and hold everything
else fixed:

1. **Same detectors for every method.** All methods start from the *identical*
   Sentinel-2 optical and Sentinel-1 SAR detections (§5.1). No method gets a better
   input point set than another.
2. **Same Stage-3 decision for every fusion method.** `no_alignment`,
   `traditional_icp` and `proposed` all feed their (aligned or unaligned) point
   sets into the *same* cross-modal validation and the *same* confidence threshold
   (§6). The only thing that varies is **how the SAR point set is aligned to the
   optical frame before matching**:

   | Method | Alignment step | What it represents |
   |--------|----------------|--------------------|
   | `optical_only` | — (single sensor) | no fusion at all — the "trust one sensor" baseline |
   | **`no_alignment`** | **identity (none)** | **the traditional cross-modal method *without* our idea** — overlay the two geocoded point sets as-is and take the agreement (`src/baseline.py::run_no_alignment`) |
   | `traditional_icp` | ICP | a classic registration attempt (no outlier model) |
   | `proposed` | RANSAC → CPD (ours) | our robust point-set alignment |

3. **Two evaluation regimes.**
   * **Controlled simulation with ground truth (§8.2).** Scenes are generated with
     a *known* set of true debris and a *known* SAR→optical mis-registration, then
     each detector is given realistic misses, localisation noise and
     modality-specific false positives. Because the truth is known, we compute
     **precision, recall, F1 and average precision (AP)** for every method,
     averaged over 20 random seeds, at a fixed operating point — and sweep the
     clutter load for the robustness curve. This isolates the *algorithm*.
   * **Real Nagpur lakes (§8.1, §8.3).** There is no ground-truth debris label for
     a live lake, so on real data we report what *can* be measured objectively:
     the number of debris **corroborated by both sensors**, the recovered
     registration offset, and — crucially — how the corroboration count behaves as
     the **match tolerance** is tightened (the sweep in the dashboard). Alignment
     shows its value precisely where the tolerance is tight.
4. **The reported result is the *relative ordering*,** not the absolute numbers —
   the simulation's absolute scores depend on the injected noise/clutter, but the
   ranking (ours > ICP > no-alignment) is the finding.

Everything in §8.2 is produced by `python run_demo.py` (→ `results/metrics.json`
and the figures); the real-data numbers by the acquisition pipeline over each lake.
...
**The comparison this project is about: with vs without the point-set alignment.**
Every method below shares the *same* detectors and the *same* cross-modal decision;
they differ only in how (or whether) the two point sets are aligned first.

| Method | Alignment | Precision | Recall | F1 | Average Precision |
|--------|-----------|:---:|:---:|:---:|:---:|
| optical-only | — (no fusion) | 0.75 | 0.81 | 0.78 | 0.75 |
| **no alignment (traditional)** | **none** | 0.65 | 0.12 | 0.20 | 0.52 |
| traditional (ICP) | ICP | 0.79 | 0.30 | 0.42 | 0.66 |
| **proposed (RANSAC+CPD)** | **ours** | **0.93** | **0.54** | **0.67** | **0.81** |

* **The point-set alignment is decisive.** The traditional cross-modal method
  *without* alignment overlays the two geocoded point sets as-is; the residual
  offset pushes real pairs outside the match radius, so it recovers almost nothing
  (**recall 0.12, F1 0.20**). Adding our alignment lifts the *same* fusion to
  **recall 0.54, F1 0.67** — a **~4.6× recall** and **~3.4× F1** gain from that one
  step. (`src/baseline.py::run_no_alignment`.)
* **It also beats classic ICP alignment** on every metric (F1 0.67 vs 0.42, AP 0.81
  vs 0.66): ICP has no outlier model and is dragged off by each sensor's
  modality-specific false alarms, where RANSAC+CPD is robust.
* Precision holds as clutter grows where the baselines collapse (see robustness).

> **▶ Live comparison dashboard** — an interactive write-up of this with/without-
> alignment comparison (real Nagpur overlay + charts) is generated by
> [`docs/build_dashboard.py`](docs/build_dashboard.py) into
...
### 8.3 With vs without alignment on **real, live** data

Running the fully-real pipeline (latest clear Sentinel-2 × latest Sentinel-1D GRD,
both live) over three Nagpur lakes, the recovered residual offset between the two
detection point sets is genuine but small (real products are already fairly well
geolocated):

| Lake | S2 date | S1 date | Optical det. | Radar det. | Offset recovered |
|------|:-------:|:-------:|:---:|:---:|:---:|
| Ambazari | 2026-07-16 | 2026-08-10 | 41 | 31 | ~50 m |
| Gorewada | 2026-05-30 | 2026-08-10 | 47 | 25 | ~19 m |
| Futala   | 2026-05-27 | 2026-08-10 | 49 | 15 | ~130 m |

The alignment's value appears when the **match tolerance is tight** (a *precise*,
low-false-match acceptance radius). Sweeping the tolerance on Ambazari (offset
~50 m), the traditional no-alignment overlay loses the corroborations the offset
pushes out of range, while our alignment keeps them:

| Match tolerance | 20 m | 30 m | 40 m | 50 m | 60 m | 80 m |
|-----------------|:---:|:---:|:---:|:---:|:---:|:---:|
| no alignment (traditional) | 1 | 1 | 1 | 2 | 5 | 5 |
| **with alignment (ours)** | **3** | **3** | **3** | **3** | 3 | 5 |

At a tight **30 m** tolerance the alignment confirms **3× more** debris (3 vs 1). At
a loose tolerance the two converge — but a loose tolerance also admits false
matches, so the alignment is what lets you match *precisely*. (Interactive version
in the dashboard's "live Nagpur data" panel above; data in
`results/real_alignment_compare.json` and `results/real_radius_sweep.json`.)

---

## 9. Conclusions

1. **The point-set alignment is what makes cross-modal fusion work.** The
   traditional method that fuses the two sensors *without* aligning the point sets
   barely corroborates anything (recall 0.12, F1 0.20); adding our alignment lifts
   the identical fusion to recall 0.54 / F1 0.67 — a ~4.6× recall gain from that one
   step. This is the central comparison of the project.
2. **The fusion works on real Nagpur data.** Reading live Sentinel-2 and live
   Sentinel-1 over the Nagpur lakes, the pipeline detects floating matter, aligns
   the two, and produces a validated debris-confidence map.
3. **Cross-modal validation is a precision tool.** Requiring agreement between
...
4. **RANSAC+CPD beats the traditional ICP method too.** With the fusion stage held
   ...
   robustly.
5. **The registration-gated confidence keeps the method honest.** `Q_reg` prevents
...
src/
  ...
  baseline.py          traditional baselines: no-alignment fusion, ICP, and optical-only
...
docs/
  ...
  DATA.md              data-acquisition guide (S2 COG bucket, S1 via AWS/PC/Earth Engine)
  dashboard.html       self-contained "with vs without alignment" comparison UI
  build_dashboard.py   regenerates dashboard.html from results/ (no drift from code)
